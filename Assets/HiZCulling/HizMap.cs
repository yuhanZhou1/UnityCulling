...
namespace HizMapGenerator
{
    public class HizMap
    {
        ...
        private static RenderTexture _hizmap;
        ...
        public static RenderTexture hizTexture{
            get{
                return _hizmap;
            }
        }
        public void Update(ScriptableRenderContext context, Camera camera)
        {
            ...
            //blit begin
            _commandBuffer.SetComputeTextureParam(_computeShader,KERNEL_BLIT,
                ShaderConstants.InTex,ShaderConstants.CameraDepthTexture);
            _commandBuffer.SetComputeTextureParam(_computeShader,KERNEL_BLIT,
                ShaderConstants.MipTex,hizMap,0);

            _commandBuffer.SetComputeVectorParam(_computeShader,ShaderConstants.SrcTexSize,
                new Vector4(camera.pixelWidth,camera.pixelHeight,0,0));
            _commandBuffer.SetComputeVectorParam(_computeShader,ShaderConstants.DstTexSize,
                new Vector4(dstWidth,dstHeight,0,0));
#if PING_PONG_COPY
            GetTempHizMapTexture(ShaderConstants.PingTex,hizMap.width,_commandBuffer);
            _commandBuffer.SetComputeTextureParam(_computeShader,KERNEL_BLIT,
                ShaderConstants.MipCopyTex,ShaderConstants.PingTex,0);
#endif
...
#if PING_PONG_COPY
            _commandBuffer.SetComputeTextureParam(_computeShader,KERNEL_REDUCE,
                ShaderConstants.InTex,ShaderConstants.PingTex);
#else
            ...
            for (int i = 1; i < hizMap.mipmapCount; i++)
            {
                ...
                _commandBuffer.SetComputeTextureParam(_computeShader,KERNEL_REDUCE,
                    ShaderConstants.MipTex, hizMap,i);
                
#if PING_PONG_COPY
                GetTempHizMapTexture(pongTex,dstWidth,_commandBuffer);
                _commandBuffer.SetComputeTextureParam(_computeShader,KERNEL_REDUCE,
                    ShaderConstants.MipCopyTex,pongTex,0);
#endif
        ...
        public RenderTexture EnsureHizMap(Camera camera)
        {
        ...
        private RenderTexture GetTempHizMapTexture(int size, int mipCount)
        {
            var desc = new RenderTextureDescriptor(size, size, 
                RenderTextureFormat.RFloat, 0, mipCount);
